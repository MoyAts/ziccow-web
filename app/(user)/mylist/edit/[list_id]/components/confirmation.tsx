...
const Confirmation = ({ addList, setForm, form, loading }: MainProps) => {
  ...
  return (
    <div className="w-full h-fit pb-32" ref={ref}>
      <div className="text-2xl max-mobile:text-xl mb-3">
        Preview your entry before confirmation.
      </div>
      ...
      <div className="mt-5 flex flex-col gap-3">
        ...
        {form.propertyManagment == "Rental" ? (
          <Info
            detail={
              form.rentalPrice + " " + form.currency + " Per " + form.cycle
            }
            topic="Price"
          />
        ) : (
          <Info
            detail={form.sellingPrice + " " + form.currency}
            topic="Price"
          />
        )}
      ...
      <div className="flex  justify-between mt-12">
        ...
        <button
          onClick={addList}
          className={`px-5 py-2 rounded-lg ${loading ? "bg-blue-300" : "bg-mainBlue hover:bg-blue-500"}  text-white flex gap-3`}
        >
