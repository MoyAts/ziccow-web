...
const message = ({ tablet ,messages, setMessage} : Props) => {
  return (
    <div className={`${tablet && tablet == true ? "w-full" : "basis-2/3"}   relative flex flex-col mobile:ps-12 mobile:px-4 justify-between`}>
        
        <div className={`${messages.length == 0 && "hidden"} flex   w-full z-20 items-center gap-2    bg-opacity-80 `}>
            <BackIcon className="my-auto text-mainBlue text-xl cursor-pointer" onClick={() => setMessage([])}  />
            <Image src={person} className='w-14 h-14 rounded-full' alt='user' />
            <div>Chantel Shelburne</div>
        </div>

        <div className={` scroller h-fit overflow-auto  w-full flex flex-col pt-4   mobile:px-4 gap-10 pb-12 `}>
