...
const Setting = () => {
   ...
   const [updateNetworkProfile, { loading,error,data,reset}] = useMutation(UPDATE_PROFILE_PICTURE)
    ...
    <Modal
      ...
      >
        ...
        <div className='flex flex-col items-center  gap-2'>
            {
                currImg &&
                <Image src={currImg} alt='zm' className={'w-[300px]  mt-12'} width={100} height={100} />
            }
            <button
                  ...
                  <input type="file" multiple hidden placeholder="image" ref={ref} onChange={onSelect} />
                    {
                        ...
                        ?
                        <div className="">
                            <Image src={currImg ?? state.user?.profile_pic ?? ""} className=" w-[3.5em] h-[3.5em] rounded-full" width={100} height={100} alt="profile" />
                        </div>
                        :
                    ...
                    <div onClick={() =>{ref.current && ref.current.click()}} className='cursor-pointer absolute bottom-0 right-0 rounded-full flex  w-[1.5em] h-[1.5em] bg-mainBlue'>
