          ...
          <div className="flex flex-col my-4 mt-6 gap-5 text-[16px] text-lightGray">
            ...
            <div className={'flex flex-col gap-2 capitalize '}>
                <div className={'w-full  bg-white flex rounded-2xl px-2 py-1 border ' } style={{borderColor:"#DBD7DD"}}>
                    ...
                    <input onChange={({target}) => setTourType(target.value)} type={"text"} name={"name"} placeholder={"In-person touring"} className={'py-3  w-full outline-none rounded-lg '}/>
                    <InfoIcon className="text-3xl my-auto text-mainBlue -rotate-90" />
                </div>
                <div className={'w-full  bg-white flex rounded-2xl px-2 py-1 border ' } style={{borderColor:"#DBD7DD"}}>
                    <Image src={InputIcon} alt="" width={23} className={'ms-2 me-3 '}/>
                    <input onChange={({target}) => setTime(target.value)} type={"time"} name={"name"} placeholder={"5:30 PM"} className={'py-3  w-full outline-none rounded-lg appearance-none focus:outline-none'}/>
                    {/* <Image src={timeIcon} alt="" width={23} className={'w-7 my-auto text-mainBlue -rotate-90'}/> */}
                </div>
                <button onClick={submit} className="w-full py-4 text-white items-center justify-center rounded-2xl bg-mainBlue flex gap-3">
                    <p>
                        {loading ? "loading" : error ? "error" : data ? "successully added" : "Request this time"}
                    </p>
