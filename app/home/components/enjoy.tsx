"use client";

const index = () => {
  return (
    <div className="w-full bg-mainDark h-[45vh] max-tablet:h-fit tablet:max-h-[300px] flex">
      <div className="w-full max-w-[1700px]  m-auto  flex max-tablet:flex-col max-tablet:gap-4 gap-12 py-20 max-vsmall:px-5 max-tablet:px-10 px-20 text-white">
        <div className="basis-3/12 text-[25px] max-tablet:text-center">
          Enjoy quality life with our housing services.
        </div>
        <p className="basis-9/12 text- my-auto text-gray-400 max-tablet:text-center">
          Zirrow General Trading is the right choice for those of you who are
          looking for comfortable, safe and affordable housing. With our
          services, you can enjoy the privacy and comfort of living in a
          beautiful and clean environment. You can see for yourself how our
          housing services can offer beautiful and comfortable housing for you
          and your family.{" "}
        </p>
      </div>
    </div>
  );
};

export default index;
